...
test('один вход сто раз: сто одинаковых выходов', () => {
    ...
    const first = JSON.stringify(score({ findings: findings, texts_meta: meta(8), scenario: 'course', rubric: rubric, copy: copy }));
    for (let i = 0; i < 100; i++) {
        const shuffled = findings.slice().sort(function () { return 0.5 - (i % 2); });
        const again = JSON.stringify(score({ findings: shuffled, texts_meta: meta(8), scenario: 'course', rubric: rubric, copy: copy }));
...
test('добавленная тревожная находка не опускает уровень её оси', () => {
    ...
    const before = score({ findings: base, texts_meta: meta(8), scenario: 'course', rubric: rubric, copy: copy });
    for (const extra of ['urgency', 'scarcity', 'shame_doubt', 'fear_loss']) {
        for (const tid of ['t1', 't3']) {
            const after = score({ findings: base.concat([f(tid, extra, 3)]), texts_meta: meta(8), scenario: 'course', rubric: rubric, copy: copy });
...
test('десять одинаковых urgency в одном тексте: сумма не выше 3·w', () => {
    ...
    const r = score({ findings: findings, texts_meta: meta(1), scenario: 'ad', rubric: rubric, copy: copy });
...
test('насыщение: k-я находка весит 1/k', () => {
    const one = score({ findings: [f('t1', 'only_me', 2)], texts_meta: meta(1), scenario: 'course', rubric: rubric, copy: copy });
    const two = score({ findings: [f('t1', 'only_me', 2, { quote: 'a' }), f('t1', 'only_me', 2, { quote: 'b' })], texts_meta: meta(1), scenario: 'course', rubric: rubric, copy: copy });
...
test('два одинаковых текста: n_eff = 1', () => {
    ...
    const r = score({ findings: [f('t1', 'urgency', 2), f('t2', 'urgency', 2)], texts_meta: tm, scenario: 'course', rubric: rubric, copy: copy });
...
test('подходит и под Чёрную дыру, и под Кормушку: Чёрная дыра', () => {
    ...
    const r = score({ findings: findings, texts_meta: meta(8), scenario: 'course', rubric: rubric, copy: copy });
...
test('низкая уверенность при любых осях: Тёмная материя', () => {
    ...
    const r1 = score({ findings: heavy, texts_meta: meta(2), scenario: 'course', rubric: rubric, copy: copy });
    ...
    const r2 = score({ findings: heavy, texts_meta: small, scenario: 'course', rubric: rubric, copy: copy });
...
test('один короткий текст до 800 слов: Тёмная материя и уверенность «низкая»', () => {
    ...
    const r = score({ findings: [f('t1', 'urgency', 2)], texts_meta: tm, scenario: 'course', rubric: rubric, copy: copy });
...
test('много выброшенных цитат опускают уверенность на уровень', () => {
    ...
    const ok = score({ findings: findings, texts_meta: meta(8).map(function (t) { return Object.assign({}, t, { words: 500 }); }), scenario: 'course', rubric: rubric, copy: copy, raw_findings: 10, dropped_quotes: 1 });
    const bad = score({ findings: findings, texts_meta: meta(8).map(function (t) { return Object.assign({}, t, { words: 500 }); }), scenario: 'course', rubric: rubric, copy: copy, raw_findings: 10, dropped_quotes: 4 });
...
test('urgency в продающем тексте весит вдвое меньше, чем в посте', () => {
    const sale = score({ findings: [f('t1', 'urgency', 2)], texts_meta: meta(1, 'sale'), scenario: 'course', rubric: rubric, copy: copy });
    const post = score({ findings: [f('t1', 'urgency', 2)], texts_meta: meta(1, 'post'), scenario: 'course', rubric: rubric, copy: copy });
...
test('отрицательный балл хорошей оси обрезается до нуля, но находка видна', () => {
    const r = score({ findings: [f('t1', 'pseudo_science', 3)], texts_meta: meta(8), scenario: 'course', rubric: rubric, copy: copy });
...
test('сценарий меняет порядок осей', () => {
    const a = score({ findings: [], texts_meta: meta(8), scenario: 'course', rubric: rubric, copy: copy });
    const b = score({ findings: [], texts_meta: meta(8), scenario: 'therapist', rubric: rubric, copy: copy });
...
test('адвокат: до пяти пунктов, последний всегда про то, чего мы не видели', () => {
    ...
    const r = score({ findings: findings, texts_meta: saleMeta, scenario: 'course', rubric: rubric, copy: copy });
...
test('счётчики карточки', () => {
    ...
    ], texts_meta: meta(8), scenario: 'course', rubric: rubric, copy: copy });
    assert.deepEqual(r.counts, { promises: 2, deadlines: 3, sources: 1 });
});

test('в ответе нет ни процента, ни вероятности', () => {
    const r = score({ findings: [f('t1', 'urgency', 2)], texts_meta: meta(8), scenario: 'course', rubric: rubric, copy: copy });
